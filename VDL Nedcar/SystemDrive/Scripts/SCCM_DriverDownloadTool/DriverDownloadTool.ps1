<#
    ...
    .DESCRIPTION
		...
		Version 2.8 Added Remove Item(s) button to remove one or more models from the Import list prior to running the tool
					Fixed issue with progress bar not rendering correctly. 
					Added condition for SiteCode when checking for Dell models in WMI.
					Added Time Stamps next to key sections of the script
...
"@ -IgnoreWarnings | Out-Null
	...
	$StartDownloadButton_Click = {
		...
		If ($ValidationErrors -eq 0)
		{
			...
			$ProgressListBox.Items.Add("======== Starting Download Processes - $(Get-Date) ======== ")
			$ProgressListBox.Items.Add(" ")
			...
			foreach ($Model in $ImportModels)
			{
				...
				if ($Make -eq "HP")
				{
					...
					# Set XML Object
					$HPModelXML.GetType().FullNamw
					$HPModelSoftPaqs = $HPModelXML.NewDataSet.HPClientDriverPackCatalog.ProductOSDriverPackList.ProductOSDriverPack
					$HPSoftPaqSummary = $HPModelSoftPaqs | Where-Object { ($_.SystemName -like "*$Model*") -and ($_.OSName -like "*$(($OSComboBox.Text).Split(' ')[1])*$(($ArchitectureComboxBox.Text).Trim(' bit'))*") }
					$HPSoftPaq = $HPSoftPaqSummary.SoftPaqID
					$HPSoftPaqDetails = $HPModelXML.newdataset.hpclientdriverpackcatalog.softpaqlist.softpaq | Where-Object { $_.ID -eq "$HPSoftPaq" }
						...
						If (($ModelURL -ne $Null) -or ($ModelURL -ne "badLink"))
						{
							...
							if (((Test-Path -Path $("$DriverRepositoryRoot" + $Model + "\Driver Cab\" + $DriverCab)) -eq $true))
							{
								...
								if ((Test-Path -Path $DriverExtractDest) -eq $false)
								{
									...
									$ProgressListBox.Items.Add(" ")
									$ProgressListBox.Items.Add("==================== $PRODUCT DRIVER EXTRACT $(Get-Date) ====================")
									$ProgressListBox.Items.Add(" ")
								...
								if ($ImportInto -ne "Download Only")
								{
									...
									{
										$ProgressListBox.Items.Add(" ")
										$ProgressListBox.Items.Add("==================== $PRODUCT DRIVER IMPORT $(Get-Date) ====================")
										$ProgressListBox.Items.Add(" ")
									...
									{
										$ProgressListBox.Items.Add(" ")
										$ProgressListBox.Items.Add("==================== $PRODUCT DRIVER PACKAGE $(Get-Date)  ====================")
										$ProgressListBox.Items.Add(" ")
				...
				if (($ImportInto -like "*Both*") -or ($ImportInto -eq "MDT"))
				{
					...
					if ((Test-Path -Path $MDTPSLocation) -eq $true)
					{
						...
						$ProgressListBox.Items.Add(" ")
						$ProgressListBox.Items.Add("==================== $Product Driver Download $(Get-Date) ====================")
						$ProgressListBox.Items.Add(" ")
			...
			If ($RemoveLegacyDriverCheckbox.Checked -eq $true)
			{
				Set-Location -Path ($SiteCode + ":")
				$ProgressListBox.Items.Add(" ")
				$ProgressListBox.Items.Add("======== Superseded Driver Package Option Processing $(Get-Date) ======== ")
				$ProgressListBox.Items.Add(" ")
				UpdateLogWindow
		...
		if ($CleanUnusedCheckBox.Checked -eq $true)
		{
			Set-Location -Path ($SiteCode + ":")
			$ProgressListBox.Items.Add(" ")
			$ProgressListBox.Items.Add("======== Cleanup Driver Option Processing $(Get-Date) ======== ")
		...
		$ProgressListBox.Items.Add(" ")
		$ProgressListBox.Items.Add("======== Finished Processing - $(Get-Date) ======== ")
		$ProgressListBox.Items.Add(" ")
		UpdateLogWindow
	...
	$ConnectSCCMButton_Click = {
		...
		$ProgressListBox.ForeColor = "Black"
		$ProgressListBox.Items.Clear()
		$ProgressListBox.Items.Add("======== Validating SCCM Server Details $(Get-Date) ======== ")
		$ProgressListBox.Items.Add(" ")
		If ((Test-WSMan -ComputerName $SiteServer).wsmid -ne $null)
		{
			...
			$ProgressListBox.Items.Add(" ")
			$ProgressListBox.ForeColor = "Black"
			$ProgressListBox.Items.Add("======== Checking SCCM Prerequisites $(Get-Date) ======== ")
			$ProgressListBox.Items.Add(" ")
			...
			If ($ModuleName -ne $null)
			{
				...
				$ProgressListBox.Items.Add(" ")
				$ProgressListBox.Items.Add("======== Connecting to SCCM Server $(Get-Date) ======== ")
				$ProgressListBox.Items.Add(" ")
	...
	$MainForm.Text = 'SCConfgMgr.com - Driver Download Automation Tool v3.0'
	$MainForm.add_Load($MainForm_Load)
	#
	# RemoveItemsButton
	#
	$RemoveItemsButton.Font = 'Microsoft Sans Serif, 8.25pt, style=Bold'
	$RemoveItemsButton.Location = '28, 397'
...
'
	...
	$ManufacturerTab.Location = '4, 22'
	$ManufacturerTab.Name = 'ManufacturerTab'
	$ManufacturerTab.Size = '651, 164'
	...
	$labelPleaseSelectManufact.Size = '189, 14'
	...
	$DriverStorageTab.Location = '4, 22'
	$DriverStorageTab.Name = 'DriverStorageTab'
	$DriverStorageTab.Size = '651, 164'
...
'
	...
	$labelCautionYourTaskSeque.Size = '487, 14'
	...
	$labelSelectingThisOptionW.Size = '427, 14'
	...
	$labelThisScriptIsUsedAtYo.Size = '236, 14'
...
AAAOvAAADrwBlbxySQAACQlJREFUeF7tmu2ZozoMRtPG1jE9pY5pI91sAdNTro5kYdmWgRDI7I+r
...
nwAAAABJRU5ErkJggg==')
	...
	$ModelProgressLabel.Size = '108, 14'
...
#region Source: Globals.ps1
	...
	# Log File Location
	$LogFile = "$env:TEMP\SCConfigMgrDriverToolLog.txt"
	...
	function UpdateModeList
	{
		...
		if ($ProxyError -ne $true)
		{
			if ($HPCheckBox.Checked -eq $true)
			{
				...
				$HPModels = $HPModelSoftPaqs | Where-Object { ($_.OSName -like "*Windows*$(($OSComboBox.Text).split(' ')[1])*$(($ArchitectureComboxBox.Text).Split(' ')[0])*") } | Select-Object SystemName
				if ($HPModels -ne $null)
				{
