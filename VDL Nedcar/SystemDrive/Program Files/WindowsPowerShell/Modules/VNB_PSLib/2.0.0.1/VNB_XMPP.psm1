<#
.SYNOPSIS
    VNB Library - XMPP communication
...
$Global:glb_XMPPDll = 'C:\Scripts\Secdump\PS\agsXMPP.dll'

# Download XMPP SDK from http://www.ag-software.de/index.php?page=agsxmpp-sdk   

function Initialize-XMPP {
# ---------------------------------------------------------
# Initializes Global variables used for XMPP system
#
# ---------------------------------------------------------

	$LogScriptName = $myInvocation.MyCommand.Name
	$LogScriptPath = split-path -parent $MyInvocation.Scriptname
	
	$XMPPDll = $LogScriptPath + '\' + 'agsXMPP.dll'
	if(Test-Path $XMPPDll) {
		$Global:glb_XMPPDll =  $XMPPDll
	} else {
		Remove-Variable $Global:glb_XMPPDll
	}
	
	$CSSPAth = $LogScriptPath + '\' + 'css_styles.html'
	if(Test-Path $CSSPAth) {
		$Global:glb_CSSPath =  $CSSPAth
	} else {
		$Global:glb_CSSPath = $null
	}
}
...
# ---------------------------------------------------------	
	...
	$assemblyPath = $(resolve-path $Global:glb_XMPPDll )  
