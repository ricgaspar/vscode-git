...
Const LOGFILE = "Set-Mail.log"
Const ADS_PROPERTY_CLEAR = 1

If objArgs.Count() < 2 Then
	PrintMess "Wrong parameters!"
	PrintMess "Use setMail.vbs <username> <emailaddress>"
	wscript.quit()
End If

Dim strUsername, strMail
...
If Len(strDN) > 0 Then
	...
	Call SetMail(strDN, strMail)
Else
	PrintMess "ERROR: Could not find " & strUsername & " in the domain!" 
End If

'----------------------------------------------------------
Function SetMail(strDN, strMail)
	Dim nError	
	nError = 0
	
	'On Error Resume Next	
	Set objUser = GetObject("LDAP://" & strDN)
	strMailOld = objUser.Mail
	
	nError=err.number
  	If nError=0 Then
		if len(strMail) > 0 then 
			If StrComp(strMail, strMailOld)<>0 Then
				objUser.Mail = strMail
				objUser.SetInfo			
				nError=err.number
				If nError<>0 Then			
					PrintMess "ERROR: Could not set mail address! " & CStr(nError)
					PrintMess "       " & strDN
					PrintMess "       " & strMail
				Else
					PrintMess "Mail successfully changed to " & strMail
				End If
			End If
		Else
