...
Const LOGFILE = "Logboek\GroupCombinedSync.log"
...
Function Main()
	...
	If objArgs.count=1 Then
		...
		If IsArray(arrCmd) Then
			...
			For each group in arrCmd
				nGroup = nGroup + 1
				If nGroup=1 Then 
					strDNToGroup = SearchDC(group)
					If Len(strDNToGroup)>0 Then
						...
						End If
					Else
						PrintMess "ERROR: Destination group " & group & " could not be found in the domain."
						PrintMess String(50, "=")
						wscript.quit()
					End If
				End If	
				...
				If nGroup>1 Then
					strDNFromGroup = SearchDC(group)
					If Len(strDNFromGroup)>0 Then
						...
						If IsArray(arrFromMembers) Then 
							PrintMess " (member count: " & CStr(UBound(arrFromMembers)+1)
							arrFromTotal = CombineArray(arrFromTotal, arrFromMembers)
							nFromCount = UBound(arrFromTotal)+1
						End If
					Else
						PrintMess "Source group " & group & " could not be found in the domain."
						PrintMess String(50, "=")
						wscript.quit()
					End If
				End If
			...
			Else
				If nToCount = 0 Then
					...
					Call AddMembersToGroup(strDNToGroup, arrFromTotal)
				Else
					PrintMess " Searching for group differences."
				
					' Search for new members
					arrNewMembers = getNewMembers(arrFromTotal, arrToMembers)
					...
					arrDeletedMembers = GetDeletedMembers(arrFromTotal, arrToMembers)
	...
	Else 
		...
		PrintMess "       Usage: SyncCombineGroups.vbs destgroup;sourcegroup;sourcegroup"
		PrintMess String(50, "=")
		wscript.quit()
	End If
...
Function CombineArray(arrA, arrB)
	...
	If IsArray(arrA) Then		
		...
		If IsArray(arrB) Then
			nToCount = UBound(arrB)
			bFound = False
			For f=0 To nToCount
				For each a in arrA
					If Not bFound Then bFound = (StrComp(a, arrB(f))=0)
				Next
				If Not bFound Then 					
					ReDim Preserve arrResult(UBound(arrResult)+1)
					arrResult(UBound(arrResult)) = arrB(f)
				Else
					' PrintMess "Skipped: " & arrB(f)
				End If
				bFound = False
			Next
		Else
