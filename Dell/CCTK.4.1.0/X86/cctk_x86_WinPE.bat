...
::copy /Y %CCTKPATH%\*.dll    %WINPEPATH%\mount\Command_Configure\X86\HAPI
::xcopy %CCTKPATH%\HAPI\*.*    %WINPEPATH%\mount\Command_Configure\X86\HAPI /S /E /i /Y
...
:usage
echo.
echo cctk_x86_winpe.bat
echo.
echo Copyright 2009 - 2017 Dell Inc. All rights reserved.
echo.
echo Usage : cctk_x86_winpe.bat WINPEPATH CCTKPATH
echo 
echo Where:
echo   WINPEPATH   path where the Windows PE 2.0 contents are located
echo   CCTKPATH     path where Command Configure is installed
echo.
echo Example: cctk_x86_winpe.bat C:\vistape_x86 C:\Progra~1\Dell\Comman~1\
echo This installs the HAPI (Instrumentation)
echo drivers found in the C:\Progra~1\Dell\Comman~1\X86\HAPI folder to the base Windows PE image
