...
:: ***************************************************************************
REM Name: cctk_x86_winpe_3.bat
REM
REM Purpose:
REM    This script installs Dell drivers into a base Windows PE 3.0 image offline.
REM  
REM Arguments:
REM    %1 Path to local directory for customized Windows PE 3.0 image to create
REM    %2 Path to extracted Command Configure toolkit
REM	Example: cctk_x86_WinPE_3.bat C:\vistape_x86 C:\Progra~1\Dell\Comman~1\
...
@echo ----------------------------------------
@echo ~~1(cctk_x86_winpe.bat)-Check the Paths
@echo ----------------------------------------
...
%AIKTOOLS%\Servicing\dism.exe  /image=%WINPEPATH%\mount /Add-Package /PackagePath:%AIKTOOLS%\PETools\x86\WinPE_FPs\winpe-fontsupport-ja-jp.cab
%AIKTOOLS%\Servicing\dism.exe  /image=%WINPEPATH%\mount /Add-Package /PackagePath:%AIKTOOLS%\PETools\x86\WinPE_FPs\winpe-fontsupport-zh-cn.cab
%AIKTOOLS%\Servicing\dism.exe  /image=%WINPEPATH%\mount /Add-Package /PackagePath:%AIKTOOLS%\PETools\x86\WinPE_FPs\winpe-wmi.cab
%AIKTOOLS%\Servicing\dism.exe  /image=%WINPEPATH%\mount /Add-Package /PackagePath:%AIKTOOLS%\PETools\x86\WinPE_FPs\winpe-scripting.cab
%AIKTOOLS%\Servicing\dism.exe  /image=%WINPEPATH%\mount /Add-Package /PackagePath:%AIKTOOLS%\PETools\x86\WinPE_FPs\winpe-wds-tools.cab
 


@echo --------------------------------------------------------------
@echo Copy HAPI and TOOLKIT Files to the mounted image 6
@echo --------------------------------------------------------------

xcopy %CCTKPATH%\X86\HAPI\*.*    %WINPEPATH%\mount\Command_Configure\X86\HAPI /S /E /i /Y
xcopy %CCTKPATH%\X86\*.*	   %WINPEPATH%\mount\Command_Configure\X86 /S /E /i /Y
REM copy /Y %CCTKPATH%\Readme.txt    %WINPEPATH%\mount\Command_Configure\X86
...
echo X:\Command_Configure\X86\HAPI\hapint -i -k C-C-T-K -p X:\Command_Configure\X86\HAPI\ >> %WINPEPATH%\mount\windows\system32\STARTNET.CMD
...
copy %WINPEPATH%\wim\boot.wim    %WINPEPATH%\wim\cctk_x86_pe3.wim
move /Y %WINPEPATH%\wim\boot.wim %WINPEPATH%\ISO\sources\boot.wim
%AIKTOOLS%\x86\oscdimg -n -b%WINPEPATH%\etfsboot.com %WINPEPATH%\ISO %WINPEPATH%\WIM\Command_Configure_x86_3.iso

goto done
:usage
echo.
echo cctk_x86_winpe_3.bat
echo.
echo Copyright 2009 - 2017 Dell Inc. All rights reserved.
echo.
echo Usage : cctk_x86_winpe_3.bat WINPEPATH CCTKPATH
echo.
echo Where:
...
echo ***********************************************************************
echo.
echo Example: cctk_x86_WinPE_3.bat C:\winsevpe_x86 C:\Progra~1\Dell\Comman~1\
echo This installs the HAPI (Instrumentation)
echo drivers found in the C:\Progra~1\Dell\Comman~1\X86\HAPI folder to the base
echo Windows PE image in C:\winsevpe_x86.
echo.
...
:done
@echo -------------------------------
@echo (cctk_x86_winpe_3.bat)-DONE
@echo -------------------------------
set WINPEPATH=
set CCTKPATH=
set AIKTOOLS=
