...
:: ***************************************************************************
REM Name: cctk_x86_winpe_10.bat
REM
REM Purpose:
REM    This script installs Dell drivers into a base Windows PE 10.0 image offline.
REM  
REM Arguments:
REM    %1 Path to local directory for customized Windows PE 10 image to create
REM    %2 Path to extracted Command Configure toolkit
REM	Example: cctk_x86_WinPE_10.bat C:\vistape_x86 C:\Progra~1\Dell\Comman~1\
...
set CCTKPATH=%2%\X86
set WINPEPATH=%1%
set WINPECCTKPATH=%WINPEPATH%\mount\Command_Configure\X86

set WinPERoot=C:\Program Files\Windows Kits\10\Assessment and Deployment Kit\Windows Preinstallation Environment
set OSCDImgRoot=C:\Program Files\Windows Kits\10\Assessment and Deployment Kit\Deployment Tools\x86\Oscdimg

set ADKTOOLSROOT=C:\Program Files\Windows Kits\10\Assessment and Deployment Kit
set ADKTOOLS=%ADKTOOLSROOT%\Windows Preinstallation Environment\x86
set DISM=%ADKTOOLSROOT%\Deployment Tools\x86\DISM\dism.exe
set IMAGEX=%ADKTOOLSROOT%\Deployment Tools\x86\DISM\imagex.exe
...
@echo ---------------------------------------
rd /s /q %WINPEPATH%
if not exist %WINPEPATH% call "%ADKTOOLS%\..\copype.cmd" x86 "%WINPEPATH%"
...
"%DISM%" /mount-Wim /WimFile:"%ADKTOOLS%\en-us\winpe.wim" /index:1 /mountdir:"%WINPEPATH%\mount"
...
echo X:\Command_Configure\X86\HAPI\hapint -i -k C-C-T-K -p X:\Command_Configure\X86\HAPI\ >> %STARTNET%
echo echo Successfully Installed the HAPI Drivers >> %STARTNET%
echo cd X:\Command_Configure\x86>> %STARTNET%
echo echo *********************>> %STARTNET%
@echo --------------------------------------
...
"%OSCDImgRoot%\oscdimg.exe" -b%WINPEPATH%\fwfiles\etfsboot.com -n %WINPEPATH%\ISO %WINPEPATH%\WIM\Command_Configure_x86_10.iso
...
:usage
echo.
echo cctk_x86_winpe_10.bat
echo.
echo Copyright 2009 - 2017 Dell Inc. All rights reserved.
echo.
echo Usage : cctk_x86_winpe_10.bat WINPEPATH CCTKPATH
echo.
echo Where:
...
echo ***********************************************************************
echo.
echo Example: cctk_x86_WinPE_10.bat C:\winsevpe_x86 C:\Progra~1\Dell\Comman~1\"
...
:done
@echo -------------------------------
@echo (cctk_x86_winpe_10.bat)-DONE
@echo -------------------------------
set WINPEPATH=
set CCTKPATH=
set AIKTOOLS=
