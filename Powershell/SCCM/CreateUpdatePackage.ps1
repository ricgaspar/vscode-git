...
# ---------------------------------------------------------
    ...
    process {
        try {
            ...
            Echo-Log "Error: Cannot retrieve ADR set from WMI."
        }
        return $result
    }
}
...
Function Set-AutoDeploymentRules-PackageID {
    ...
    process {        
        # Retrieve all ADR for normal Software Updates
        $Rules = Get-AutoDeploymentRules -SiteServer $SiteServer -SiteCode $SiteCode | Where-Object { $_.IsServicingPlan -eq $False}
        if($Rules) {
        ...
        try { 
            ...
            if ((Get-CimInstance -CimSession $CimSession -Namespace "root\SMS\site_$($SiteCode)" -ClassName SMS_SoftwareUpdatesPackage -Filter "Name like '$($Name)'" -ErrorAction SilentlyContinue -Verbose:$false | Measure-Object).Count -eq 0) { 
                ...
                if ((Get-DuplicateInfo) -eq $false) { 
                    ...
                    Echo-Log "A Deployment Package with the specified source path already exists" 
                } 
            } 
            else { 
                Echo-Log "A Deployment Package with the name '$($Name)' already exists" 
            } 
        } 
        catch [Exception] { 
            Remove-CimSessions 
...
$logfile = "SCCM-CreateUpdatePackage-$cdtime"
...
# Set Package values
$PackageId = $null
$DeploymentPackageName = "Updates $Year-$Month"
$Description = "Created by powershell script on $Date"
$SourcePath = "\\S008\updates$\Updates-"+$Year+'-'+$Month

Echo-Log "Package: $DeploymentPackageName"
Echo-Log "Source path: $SourcePath"
...
if(Test-PathExists $SourcePath) {
    Echo-Log "Success. The source folder path exists."
	...
	Echo-Log "PackageID applied to all ADR rules: $PackageId"
    ...
    try {
        Echo-Log "Distributing new package to group '$DistributionGroupName'"
        ...
        Echo-Log "Could not distribute content of package $DeploymentPackageName"
        Echo-Log "The package may already be distributed to group '$DistributionGroupName'"
        
        try {
            Echo-Log "Starting redistribution of the package."
            # Update-CMDistributionPoint -SoftwareUpdateDeploymentPackageName $DeploymentPackageName | Out-Null
        }
        catch {
            Echo-Log "ERROR: Could not redistribute content of package $DeploymentPackageName"
        }
    }
	...
	Echo-Log "Error: Cannot create package source location: $SourcePath"
}
