...
Function Get-UptimeX {
    ...
    process {
        ...
        foreach ($Computer in $ComputerName) {
            try {
                $cnt++
                Write-Host $cnt $computer
                $hostdns = [System.Net.DNS]::GetHostEntry($Computer)
                Write-Host $host_dns
                $connect = Test-Connection -ComputerName $hostdns.HostName -ErrorAction Stop
                if ($connect) {
...
$CompLijst = [string[]]''
ForEach ($rec in $data) {
    $CompLijst += [string]$rec.Systemname
}

$Result = $CompLijst | Get-UptimeX
$Result
