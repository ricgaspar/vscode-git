    ...
    $DSComputers | ForEach-Object {         
        ...
        Start-Job -Name $JobName -ArgumentList $CompName, $dspath -ScriptBlock {
            ...
            if ($ReverseDNS.HostName -ne $CompName) {                                                                           	
                $strOutput += "[$CompName] ERROR: A reverse DNS lookup failure occured. The IP address does not belong to this computer.`n"
            }
            else { 
                ...
                if ($Connected) {                        						    
                    ...
                    $strOutput += "[$CompName] ERROR:  Cannot connect to $CompName. IP address: $IPAddress`n"
