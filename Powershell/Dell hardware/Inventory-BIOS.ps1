...
$UDL = Read-UDLConnectionString $Global:glb_UDL
$SQLconn = New-UDLSQLconnection $UDL
$query = "select Systemname, DN, ApplicDomain from vw_VNB_DOMAIN_COMPUTERS where Systemname like 'VDLNC%' and DN like '%,OU=ALCBS%' order by ApplicDomain, Systemname"
...
if ($data -ne $null) {
    ...
    ForEach ($rec in $data) {
        ...
        Try {
            ...
            $Result = Join-Object -Left $OS -Right $BIOS -LeftJoinProperty PSComputername -RightJoinProperty PSComputername
            $COMPUTER = Get-CimInstance -ClassName Win32_ComputerSystem -ComputerName $Computername -ErrorAction SilentlyContinue | Select-Object PSComputername, Model
            $Result = Join-Object -Left $Result -Right $Computer -LeftJoinProperty PSComputername -RightJoinProperty PSComputername

            $collectionVariable.Add($Result) | Out-Null
            write-host "[$cntr of $Count]: $Computername : OK."
        }
        Catch {

            #$collectionVariable.Add($errobject) | Out-Null
            write-host "[$cntr of $Count]: $Computername : Error: could not connect!"
        }
    }
    Write-Host "Done."
    Write-Host "Total queried systems: $cntr"
}
else {
...
$collectionVariable | Export-CSV -Delimiter ';' -Path C:\Temp\computers.csv -Force